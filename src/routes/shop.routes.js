...
function cartDetails(cart) {
  ...
  const items = products.map(p => ({ product:p, qty: Math.min(cart[p.id], p.stock), subtotal: p.price * Math.min(cart[p.id], p.stock) })).filter(i => i.qty > 0);
...
router.get('/search', (req, res) => {
  ...
  let products = [];
  if (q) products = db.prepare(`SELECT * FROM products WHERE active=1 AND name LIKE ? ORDER BY id DESC`).all(`%${q}%`);
...
router.get('/cart', (req, res) => { const { items, total } = cartDetails(getCart(req)); res.render('cart', { items, total }); });
...
router.get('/checkout', requireCustomer, (req, res) => {
  ...
  res.render('checkout', {
    items, total,
    shopCard: { number: getSetting('shop_card_number', process.env.SHOP_CARD_NUMBER||''), owner: getSetting('shop_card_owner', process.env.SHOP_CARD_OWNER||'') },
    error: null,
  });
});
